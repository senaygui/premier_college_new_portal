ActiveAdmin.register AddAndDrop do
  ...
  permit_params :student_id, :academic_calendar_id, :semester_registration_id, :department_id, :program_id, :section_id,
                :semester, :year, :registrar_approval, :registrar_name, :registrar_date_of_response, :advisor_approval, :advisor_name, :advisor_date_of_response, :created_by, :updated_by, add_and_drop_courses_attributes: %i[id add_and_drop_id course_id add_or_drop advisor_approval advisor_name advisor_date_of_response created_by updated_by _destroy]
  ...
  form do |f|
    ...
    if f.object.new_record? && ((current_admin_user.role == 'registrar head') || (current_admin_user.role == 'admin'))
      ...
      f.object.add_and_drop_courses << AddAndDropCourse.new if f.object.add_and_drop_courses.empty?
    ...
    unless f.object.new_record?
      if (params[:page_name] == 'advisor_approve') || ((current_admin_user.role == 'department head') || (current_admin_user.role == 'admin'))
        ...
        f.inputs 'Advisor Approval' do
          f.input :advisor_approval, as: :select, collection: %w[pending approved denied],
                                     include_blank: false
  ...
  show title: proc { |add_and_drop|
                truncate(
                  "#{add_and_drop.student.first_name.upcase} #{add_and_drop.student.middle_name.upcase} #{add_and_drop.student.last_name.upcase}", length: 50
                )
              } do
    columns do
            ...
            column do
              table(class: 'form-table') do
                ...
                tr class: 'form-table__row' do
                  ...
                  td "#{if add_and_drop.advisor_date_of_response.present?
                          add_and_drop.advisor_date_of_response.strftime('%b %d, %Y')
                        end}",
                     class: 'form-table__col'
                end
              end
            end
            column do
              table(class: 'form-table') do
                ...
                tr class: 'form-table__row' do
                  th 'Responded At', class: 'form-table__col'
                  td "#{if add_and_drop.registrar_date_of_response.present?
                          add_and_drop.registrar_date_of_response.strftime('%b %d, %Y')
                        end}",
                     class: 'form-table__col'
                end
              end
